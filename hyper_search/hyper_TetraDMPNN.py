...
def optimize(trial, args):
    ...
    dataset = MoleculeDataset(root=data_dir, file_name=file_name, label_col=args.label,graph_type='tetra',transform=get_mask_chiral)
    split_idx = get_split_idx(len(dataset),val_ratio=0.1,test_ratio=0.1,seed=args.seed)
    train_loader = DataLoader(dataset[split_idx['train']],batch_size=batch_size,shuffle=True)
    val_loader = DataLoader(dataset[split_idx['val']],batch_size=batch_size,shuffle=True)
    test_loader = DataLoader(dataset[split_idx['test']],batch_size=batch_size,shuffle=False)

    # construct model
    device = torch.device("cuda:0" if torch.cuda.is_available() and args.use_gpu else "cpu")
    model = TetraDMPNN(gnn_hidden_size, dataset.atom_feat_dim, dataset.bond_feat_dim, depth,\
                       drop_rate=dropout, graph_pool=graph_pool,tetra_message='tetra_permute_concat').to(device)
