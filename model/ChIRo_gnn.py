...
def get_local_structure_map(psi_indices):
    ...
    return LS_map.to(psi_indices.device), alpha_indices.to(psi_indices.device)
class MLP(nn.Module):
...
class GraphNodeEmbedder(nn.Module):
    def __init__(self, EConv_in_channels, EConv_out_channels, EConv_mlp, GAT_out_channels, GAT_N_heads, EConv_bias,
                 GAT_bias, dropout):
...
class ChiralMessagePassing(nn.Module):
    def __init__(self, F_H, F_alpha, Chiral_EConv_MLP_hidden_sizes=[64], Chiral_GAT_N_layers=2, Chiral_GAT_N_heads=1,
                 Chiral_EConv_bias=True, Chiral_GAT_bias=True, dropout=0.0):
...
class InternalCoordinateEncoder(nn.Module):
    def __init__(self, F_z_list, F_H, hidden_sizes_D, hidden_sizes_phi, hidden_sizes_c, hidden_sizes_sinusoidal_shift,
                 hidden_sizes_alpha, activation_dict, biases, dropout, c_coefficient_normalization='sigmoid',
                 reduction='sum'):
        ...
        self.Encoder_D = MLP(  # Bond Distance Encoder
            input_size=2 * F_H + 1,
        ...
        self.Encoder_phi = MLP(  # Bond Angle Encoder
    ...
    def forward(self, H_embeddings, distances, distance_indices, phis, phi_indices, psis, psi_indices, node_map, LS_map,
                alpha_indices):
        ...
        i, j, k = phi_indices
        hi_hj_hk_phi_tensor_forward = torch.cat(
            [H_embeddings[i], H_embeddings[j], H_embeddings[k], torch.cos(phis.unsqueeze(1)),
             torch.sin(phis.unsqueeze(1))], dim=1)
        hi_hj_hk_phi_tensor_reverse = torch.cat(
            [H_embeddings[k], H_embeddings[j], H_embeddings[i], torch.cos(phis.unsqueeze(1)),
             torch.sin(phis.unsqueeze(1))], dim=1)
        z_phi = self.Encoder_phi(hi_hj_hk_phi_tensor_forward) + self.Encoder_phi(hi_hj_hk_phi_tensor_reverse)

        i, j, k, l = psi_indices
        hi_hj_hk_hl_tensor_forward = torch.cat([H_embeddings[i], H_embeddings[j], H_embeddings[k], H_embeddings[l]],
                                               dim=1)
        hi_hj_hk_hl_tensor_reverse = torch.cat([H_embeddings[l], H_embeddings[k], H_embeddings[j], H_embeddings[i]],
                                               dim=1)

        c_tensor = self.Encoder_c(hi_hj_hk_hl_tensor_forward) + self.Encoder_c(hi_hj_hk_hl_tensor_reverse)

        phase_shift_linear_output = (
                    self.Encoder_sinusoidal_shift(hi_hj_hk_hl_tensor_forward) + self.Encoder_sinusoidal_shift(
                hi_hj_hk_hl_tensor_reverse))  # [linear_cos, linear_sin]

        # L2 normalization
        phase_shift_norm = torch.linalg.norm(phase_shift_linear_output, ord=2, dim=1, keepdim=True)
        phase_shift_linear_output_normalized = phase_shift_linear_output / torch.max(phase_shift_norm,
                                                                                     torch.cuda.FloatTensor(1).fill_(
                                                                                         1e-12).squeeze()) if torch.cuda.is_available() else phase_shift_linear_output / torch.max(
            phase_shift_norm, torch.FloatTensor(1).fill_(1e-12).squeeze())
        ...
        if self.c_coefficient_normalization:
            if self.c_coefficient_normalization == 'softmax':
                normalized_c_tensor = torch_scatter.composite.scatter_softmax(c_tensor.squeeze(), LS_map.squeeze(),
                                                                              dim=0).unsqueeze(1)
            ...
            scaled_torsions = torch.cat([torch.cos(psis.unsqueeze(1)) * phase_cos.unsqueeze(1) - torch.sin(
                psis.unsqueeze(1)) * phase_sin.unsqueeze(1),
                                         torch.sin(psis.unsqueeze(1)) * phase_cos.unsqueeze(1) + torch.cos(
                                             psis.unsqueeze(1)) * phase_sin.unsqueeze(1)], dim=1) * normalized_c_tensor

        else:
            scaled_torsions = torch.cat([torch.cos(psis.unsqueeze(1)) * phase_cos.unsqueeze(1) - torch.sin(
                psis.unsqueeze(1)) * phase_sin.unsqueeze(1),
                                         torch.sin(psis.unsqueeze(1)) * phase_cos.unsqueeze(1) + torch.cos(
                                             psis.unsqueeze(1)) * phase_sin.unsqueeze(1)], dim=1) * c_tensor
        ...
        return z, phase_shift_norm, z_alpha, c_tensor, phase_cos, phase_sin, torch.cat(
            [torch.cos(psis.unsqueeze(1)), torch.sin(psis.unsqueeze(1))], dim=1), pooled_sums


class Encoder(nn.Module):
    def __init__(self, F_z_list, F_H, F_H_embed, F_E_embed, F_H_EConv, layers_dict, activation_dict, GAT_N_heads=1,
                 chiral_message_passing=False, CMP_EConv_MLP_hidden_sizes=[64], CMP_GAT_N_layers=2, CMP_GAT_N_heads=1,
                 c_coefficient_normalization=None, encoder_reduction='mean', output_concatenation_mode='none',
                 EConv_bias=True, GAT_bias=True, encoder_biases=True, dropout=0.0):
        ...
        activation_dict = deepcopy(activation_dict)
        ...
        GAT_out_channels.append(
            self.F_H)  # the final GAT layer must output node embeddings with the target dimesion, F_H
    ...
    def forward(self, data):
        ...
        psi_indices = data.dihedral_angle_index
        ...
        latent_vector, phase_shift_norm, z_alpha, c_tensor, phase_cos, phase_sin, sin_cos_psi, sin_cos_alpha = self.InternalCoordinateEncoder(
            H_embeddings, distances, distance_indices, phis, phi_indices, psis, psi_indices, node_map, LS_map,
            alpha_indices)
